...
import sys
sys.path.append('..')
import CNN.original_cnn as co
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

# param config
flags = tf.app.flags
flags.DEFINE_integer('iter', 10000, 'Iteration to train.')
flags.DEFINE_integer('batch_size', 488, 'The size of each batch.')
...
def discriminator(x):
    out, D_W1, D_b1, D_W2, D_b2, D_W3, D_b3 = co.inference(x, 20, 24, 1, 100)
    return out, D_W1, D_b1, D_W2, D_b2, D_W3, D_b3

G_sample = generator(z)
D_real, D_W1, D_b1, D_W2, D_b2, D_W3, D_b3 = discriminator(X)
D_fake, _, _, _, _, _, _ = discriminator(G_sample)
...
def main(_):
    if FLAGS.is_train:
        ...
        for it in range(FLAGS.iter):
            ...
            D_real_acc_curr_1, D_fake_acc_curr_1, last_value = sess.run([D_real_acc, D_acc, l],
                                                                                                        feed_dict={X: X_mb, z: z_sample})
            # print('acc_x_mb', X_mb)
            ...
            if it % 1000 == 0:
                ...
                # print(samples)
                sio.savemat(FLAGS.model_path + '/data' + str(it) + '.mat', {'g_sample': samples})
