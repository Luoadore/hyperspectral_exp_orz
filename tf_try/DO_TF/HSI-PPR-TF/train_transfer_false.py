...
flags.DEFINE_string('ckpt_dir', 'baseline/1st/ksc_0520_s80/', 'ckpt of model.')
...
def placeholder_inputs(batch_size):
    ...
    data_placeholder = tf.placeholder(tf.float32,
                                      shape=(None, oc.BANDS_SIZE * FLAGS.conv1_stride))
...
def run_training():
    ...
    train_data = data['t_' + FLAGS.train_ratio]
    train_label = np.transpose(data['t_' + FLAGS.train_ratio + '_l'])
    # source_test_data = data['t_' + FLAGS.test_ratio]
    # source_test_label = np.transpose(data['t_' + FLAGS.test_ratio + '_l'])
    target_test_data = data['t_' + FLAGS.test_ratio]
    target_test_label = np.transpose(data['t_' + FLAGS.test_ratio + '_l'])
    ...
    sio.savemat(data_set['train_dir'] + FLAGS.save_name + 'transfer_data.mat', {
    # 'train_data': train_data, 'train_label': dp.decode_onehot_label(train_label, oc.NUM_CLASSES), 'train_pos': train_pos,
        ...
        #'source_test_acc': s_test_acc_steps,
        ...
        #'source_test_prediction': s_test_prediction,
        'target_test_prediction': t_test_prediction})
